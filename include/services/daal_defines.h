...
#define DAAL_CHECK_BREAK(cond)             { if((cond))  break;  }
#define DAAL_CHECK_STATUS_OK(cond, status) { if(!(cond))  return status;  }
#define DAAL_CHECK_COND_ERROR(cond, status, error) { if(!(cond))  (status).add(error); }
